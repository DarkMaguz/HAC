...
bool is_file( const char *path )
{
	...
	return S_ISREG( x.st_mode );// || S_IFLNK( x.st_mode );
...
/*bool dexist( const char *path )
{
	...
	if ( path[it - 1] == DIR_CHAR )
		it--;
	
	static const char tmp_file[] = ".test";
	
	char tmp_path[it + 5 + 1];
	
	for ( int i = 0; i < it; i++ )
		tmp_path[i] = path[i];
	
	tmp_path[it] = DIR_CHAR;
	
	it++;
	
	for ( int i = 0; i < 5; i++ )
		tmp_path[it+i] = tmp_file[i];
	              
	FILE *fs = fopen( tmp_path, "r" );
