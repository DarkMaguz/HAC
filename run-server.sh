...
# Launch services.
docker run --name hacdb -d -p 3306:3306 -e MYSQL_ROOT_PASSWORD=root -e MYSQL_DATABASE=hac01 -e MYSQL_USER=${MYSQL_USER} -e MYSQL_PASSWORD=${MYSQL_PASSWORD} -v $(pwd)/sql:/docker-entrypoint-initdb.d -v $(pwd)/datadir:/var/lib/mysql mysql/mysql-server:5.7
...
# Run the server.
docker run -it --name hac-server -v $(pwd)/log:/usr/app/log --link hacdb:hacdb --link smtp:smtp -p 4337:4337 darkmagus/hac-server
